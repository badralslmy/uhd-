...
const driveFileIdInput = document.getElementById('driveFileId'); // حقل Google Drive File ID
...
contentForm.addEventListener('submit', async (e) => {
    ...
    const driveFileId = document.getElementById('driveFileId').value; // File ID من Google Drive
    const posterPortraitFile = posterPortraitInput.files[0];
    const posterLandscapeFile = posterLandscapeInput.files[0];

    // التحقق من ملء جميع الحقول
    if (!title || !type || !description || !year || !rating || !posterPortraitFile || !posterLandscapeFile || !driveFileId) {
    ...
    readerPortrait.onload = async (e) => {
        readerLandscape.onload = async (e2) => {
            const newContent = {
                ...
                driveFileId, // إضافة File ID من Google Drive
...
const editContent = async (id) => {
    ...
    if (content) {
        ...
        document.getElementById('driveFileId').value = content.driveFileId || ''; // تعبئة File ID من Google Drive
        ...
        submitBtn.onclick = async () => {
            ...
            const updatedContent = {
                ...
                driveFileId: document.getElementById('driveFileId').value, // تحديث File ID من Google Drive
...
async function addContentFromTMDB(result) {
    ...
    const content = {
        ...
        driveFileId: null, // يمكن إضافة File ID يدويًا لاحقًا
